	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		if (column.fieldname === "party" && data && data.payment_average < 500) {
			if (data.available_credit > 0) {
				...
				value = "<div style='display: flex; align-items: baseline'>" +
					"<button style='background:black; width: 10px; height: 10px; border-radius: 99px; padding: 0; border: none;'" + "value='" + data.party + "' onclick='createSalesOrder(this.value)'></button>" + "<div style='width: 18px;'></div>"
					+ value +
					"</div>";
			}
			...
			if (data.available_credit > 1500) {
				...
				value = "<div style='display: flex; align-items: baseline'>" +
					"<button style='background:orange; width: 10px; height: 10px; border-radius: 99px; padding: 0; border: none;'" + "value='" + data.party + "' onclick='createSalesOrder(this.value)'></button>" + "<div style='width: 18px;'></div>"
					+ value +
					"</div>";
			}
		}
		...
		} else if (column.fieldname === "payment_average" && data && data.payment_average >= data.promised_payment && data.payment_average >= 500) {
...
function show_available_credit_test_dialog (current_payment_avarage, closing_balance) {
	let dialog = new frappe.ui.Dialog({
		...
		async primary_action(values) {
			const new_payment_avarage = current_payment_avarage + (values['amount'] / 13)

			dialog.set_value('new_payment_avarage', new_payment_avarage)

			let credit_limits = []
			let closer_avarage = null

			let rate = await frappe.db.get_doc('Rate Settings')
			rate['options'].reverse().forEach(rata => {
				console.log(rata['max_credit'])

				if (new_payment_avarage >= rata['weekly_payment']) {
