			...
			if (
				...
				entries = frappe.db.get_list(
					...
					filters=[
						['party_type', '=', "Customer"],
						['party', '=', party],
						['posting_date', 'between', (frappe.utils.add_months(self.filters.from_date, -1), frappe.utils.add_months(self.filters.to_date, -1))]
					]
				)
				for entry in entries:
					total_previous_amount += entry.paid_amount
